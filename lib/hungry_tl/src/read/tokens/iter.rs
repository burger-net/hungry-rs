...
impl<'a> Tokens<'a> {
    ...
    fn advance(&mut self, offset: usize) {
        ...
        self.contents = trimmed;
        self.position += offset + content.len() - trimmed.len();
    }
        ...
        loop {
            match self.iterator.next()? {
                Either::Content { range } => {
                    ...
                    self.position = range.start + content.len() - trimmed.len();
                ...
                Either::Comment { comment, ended } => {
                    self.position = comment.range.start + comment.variant.start().len();
